...
fn path_key(p: &str) -> String {
    Path::new(p).to_string_lossy().replace('\\', "/")
}
...
pub(crate) fn check_read(path: &str) -> Result<(), String> {
    ...
    let record = match state.get(&key) {
        ...
        None => {
            return Err(format!(
                "{} has not been read yet. Read the file before editing it.",
                path
            ))
        }
    };
    ...
    match current.modified() {
        Ok(mtime) if mtime != record.mtime => Err(format!(
            "{} was modified since last read. Re-read before editing.",
            path
        )),
        Ok(_) => Ok(()),
