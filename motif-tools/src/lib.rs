...
pub(crate) const SKIP_DIRS: &[&str] = &[
    ".git", ".svn", ".hg", "target", "node_modules", "build", "dist",
    "__pycache__", ".cache", ".next", ".nuxt", "vendor",
];

pub(crate) const BINARY_EXTENSIONS: &[&str] = &[
    "exe", "dll", "so", "dylib", "png", "jpg", "jpeg", "gif", "ico",
    "pdf", "zip", "tar", "gz", "bz2", "class", "pyc", "wasm",
    "mp3", "mp4", "avi", "mov",
];
...
pub(crate) fn is_binary_extension(p: &Path) -> bool {
    p.extension().and_then(|e| e.to_str()).map(|e| BINARY_EXTENSIONS.contains(&e)).unwrap_or(false)
}

pub(crate) fn is_skip_dir(name: &str) -> bool { SKIP_DIRS.contains(&name) }
