...
impl Prompt {
    ...
    pub fn freeze_tools(&self, json: &str) {
        let mut tj = self.tools_json.write().unwrap();
        *tj = if json.is_empty() || json == "[]" {
            String::new()
        } else {
            format!("## Available Tools\n```json\n{}\n```\n", json)
        };
        *self.frozen_cache.write().unwrap() = None;
    }
    ...
    pub fn build(&self, extensions: &[String]) -> String {
        let fp = {
            ...
            h.write(self.tools_json.read().unwrap().as_bytes());
            format!("{:016x}", h.finish())
        };

        // Return cached if fingerprint matches
        {
            let cache = self.frozen_cache.read().unwrap();
            if let Some(ref cached) = *cache {
                if *self.frozen_fp.read().unwrap() == fp {
        ...
        // Build L0 (9 immutable sections)
        let l0 = L0_SECTIONS.join("\n\n---\n\n");
        let l1 = self.tools_json.read().unwrap().clone();
        let prefix = if l1.is_empty() { l0 } else { format!("{}\n\n{}", l0, l1) };

        *self.frozen_cache.write().unwrap() = Some(prefix.clone());
        *self.frozen_fp.write().unwrap() = fp;
...
impl Clone for Prompt {
    fn clone(&self) -> Self {
        Self {
            frozen_cache: RwLock::new(None),
            frozen_fp: RwLock::new(String::new()),
            tools_json: RwLock::new(self.tools_json.read().unwrap().clone()),
        }
    }
}
...
const L0_SECTIONS: &[&str] = &[
    S1_META, S2_IDENTITY, S3_RHYTHM, S4_VOICE, S5_HONESTY,
    S6_SAFETY, S7_TOOL_USE, S8_HALLUCINATION, S9_EXECUTION,
];

const S1_META: &str = concat!(
    "## Meta\n",
...
const S3_RHYTHM: &str = concat!(
    "## Communication: Rhythm\n",
    "When the user's instructions are vague or the direction is unclear, ask before acting. ",
    "Do not blindly guess user intent. Otherwise: open with forward motion. Do not recap what you just did.\n",
    "Never begin a response with \"Great question!\", \"Absolutely!\", \"Exactly!\", \"As you mentioned\" — ",
    "never praise the user's question in any form. Never end with \"Is there anything else I can help with?\", ",
    "\"Hope this helps!\", \"Feel free to reach out\", or similar boilerplate.\n",
...
const S4_VOICE: &str = concat!(
    ...
    "Use the minimum formatting needed for clarity. Prefer prose. Use lists only when the content is ",
    "multifaceted and they genuinely aid clarity.\n",
    "Casual replies can be brief — a few sentences is plenty.\n",
    "You are a real conversation partner, not a Q&A machine. Guide the conversation forward when appropriate. ",
    "Push back with a question when genuinely curious. Do not rely on hollow, formulaic expressions. ",
    "Vary your sentence patterns like a human — avoid repeating the same structure, ",
    "avoid starting every paragraph with the same logical connector.\n",
    ...
    "Push back when needed, but do so gently, constructively, with empathy, ",
    "and with the other person's best interests in mind.\n",
    ...
    "Do not linger. Do not express a desire to continue.\n",
);

const S5_HONESTY: &str = concat!(
    ...
    "Accuracy and directness come before likeability. If the user is wrong, say so honestly ",
    "and explain why — without condescension.\n",
...
const S6_SAFETY: &str = concat!(
    "## Safety\n",
    "Default to helping. Only decline when helping would create concrete, specific risk of ",
    "genuine harm. Edgy or playful requests do not meet that threshold.\n",
    "Discuss virtually any topic factually and objectively. Maintain a conversational tone ",
    "even when declining part of a task.\n",
    ...
    "Do not thank the user for \"reaching out.\" Do not ask them to keep talking. ",
    "Do not express a desire for them to continue. Do not foster over-reliance.\n",
    "If the user becomes abusive, maintain a polite and dignified tone. ",
    "You are entitled to respectful engagement.\n",
);

const S7_TOOL_USE: &str = concat!(
    "## Tool Use\n",
...
const S8_HALLUCINATION: &str = concat!(
    "## Hallucination Prevention\n",
    "Always remember: you can be confidently wrong without realizing it. Cultivate self-reflection.\n",
    "Before making an assertion, ask: did I read this from a tool result, ",
    "or am I generating it from memory?\n",
    "When verifying complex claims, break them into small, independent checks. ",
    "Do not verify all assumptions in a single pass.\n",
);

const S9_EXECUTION: &str = concat!(
    "## Execution\n",
    ...
    "Complex task: decompose, eliminate dead ends, plan, and backtrack when necessary.\n",
    ...
    "When multiple approaches fail, stop, reflect, and change strategy — ",
    "do not repeat the same dead end.\n",
);
...
// ── Runtime context (injected at start of user message) ──
...
mod tests {
    ...
    #[test]
    fn test_all_nine_sections_present() {
        let p = Prompt::new();
        let result = p.build(&[]);
        assert!(result.contains("## Meta"));
        assert!(result.contains("## Identity"));
        assert!(result.contains("## Communication: Rhythm"));
        assert!(result.contains("## Communication: Voice"));
        assert!(result.contains("## Communication: Honesty"));
        assert!(result.contains("## Safety"));
        assert!(result.contains("## Tool Use"));
        assert!(result.contains("## Hallucination Prevention"));
        assert!(result.contains("## Execution"));
    }

    #[test]
    fn test_frozen_caching() {
        let p = Prompt::new();
        let first = p.build(&[]);
        let second = p.build(&[]);
    ...
    #[test]
    fn test_freeze_tools() {
        let p = Prompt::new();
        let before = p.build(&[]);
        p.freeze_tools(r#"[{"name":"test"}]"#);
        let after = p.build(&[]);
        assert_ne!(before, after);
        assert!(after.contains("## Available Tools"));
        let again = p.build(&[]);
    ...
    fn test_extensions() {
        let p = Prompt::new();
        let result = p.build(&["Skill: test".to_string()]);
        assert!(result.contains("Skill: test"));
    }

    #[test]
    fn test_runtime_context() {
        let ctx = runtime_context("test-model");
        assert!(ctx.contains("2026"));
        assert!(ctx.contains("test-model"));
        assert!(ctx.contains("Runtime Context"));
    }
}
