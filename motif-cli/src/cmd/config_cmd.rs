...
impl Command for ConfigCmd {
    ...
    fn desc(&self) -> &'static str { "Show current config" }
    async fn run(&self, _: &mut Agent, _: &str, cfg: &Config, _: &Registry) -> Outcome {
        ...
        if let Some(ref e) = cfg.thinking_effort { println!("thinking:    {}", e); }
        if let Some(ref extra) = cfg.extra_body {
            println!("extra_body:  {}", serde_json::to_string_pretty(extra).unwrap_or_default());
        }
        Outcome::Continue
    }
}
