...
pub struct InfiniteHistory { messages: Vec<TimedMessage> }
impl InfiniteHistory { pub fn new() -> Self { Self { messages: vec![] } } }
impl Default for InfiniteHistory { fn default() -> Self { Self::new() } }
impl History for InfiniteHistory {
    fn add(&mut self, m: TimedMessage) { self.messages.push(m); }
    fn get_all(&self) -> &[TimedMessage] { &self.messages }
    fn clear(&mut self) { self.messages.clear(); }
}

pub struct BoundedHistory { messages: Vec<TimedMessage>, capacity: usize }
impl BoundedHistory { pub fn new(n: usize) -> Self { Self { messages: vec![], capacity: n.max(1) } } }
