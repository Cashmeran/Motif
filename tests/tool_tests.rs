...
use motif::tools::{bash, read, search, write};
use motif::Tool;
use std::fs;
use std::path::Path;
use std::sync::Arc;
...
fn test_search_filename_mode() {
    ...
    let result = call_tool(&registered, r#"{"query":"*.rs","mode":"filename","path":"src"}"#);
...
fn test_search_content_mode() {
    let (_, registered) = search::register().into_parts();
    let result = call_tool(&registered, r#"{"query":"StopCondition","mode":"content","path":"src","glob":"*.rs"}"#);
...
fn test_search_files_with_matches() {
    let (_, registered) = search::register().into_parts();
    let result = call_tool(&registered, r#"{"query":"pub struct Agent","mode":"files_with_matches","path":"src","glob":"*.rs"}"#);
...
fn test_search_count() {
    let (_, registered) = search::register().into_parts();
    let result = call_tool(&registered, r#"{"query":"fn","mode":"count","path":"src","glob":"*.rs"}"#);
...
fn test_search_pagination() {
    ...
    let result = call_tool(&registered, r#"{"query":"use","mode":"filename","path":"src","head_limit":2,"offset":0}"#);
    // Should either contain results or a truncation message
    assert!(!result.is_empty());
    // head_limit=0 should return all
    let all = call_tool(&registered, r#"{"query":"use","mode":"filename","path":"src","head_limit":0}"#);
    assert!(!all.is_empty());
}
...
fn test_search_case_insensitive() {
    let (_, registered) = search::register().into_parts();
    let result1 = call_tool(&registered, r#"{"query":"stopcondition","mode":"content","path":"src","glob":"*.rs","ignore_case":true}"#);
...
fn test_search_empty_query() {
    let (_, registered) = search::register().into_parts();
    let result = call_tool(&registered, r#"{"query":"","path":"src"}"#);
