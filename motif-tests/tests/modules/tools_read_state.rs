...
fn test_read_then_edit() {
    ...
    let res = common::call_tool(
        &e,
        r#"{"file_path":"_trs.txt","old_string":"orig","new_string":"mod"}"#,
    );
    assert!(res.contains("Edited"), "{}", res);
    fs::remove_file("_trs.txt").ok();
}
#[test]
fn test_write_new_no_read() {
...
#[test]
fn test_mtime_change_after_read_blocked() {
    ...
    let res = common::call_tool(
        &e,
        r#"{"file_path":"_trs_mt.txt","old_string":"modified externally","new_string":"x"}"#,
    );
    assert!(
        res.contains("modified since") || res.contains("has not been read"),
        "Should reject edit after external modification: {}",
        res
    );
