...
fn test_bash_error_command() {
    let (_, tool) = motif_tools::bash::register().into_parts();
    let result = common::call_tool(&tool, r#"{"command":"nonexistent_command_xyz 2>&1","timeout_ms":5000}"#);
...
fn test_bash_timeout() {
    ...
    let result = common::call_tool(&tool, &format!(r#"{{"command":"{}","timeout_ms":500}}"#, cmd));
