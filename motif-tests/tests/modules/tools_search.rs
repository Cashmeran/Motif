...
fn test_search_filename_mode() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"*.rs","mode":"filename","path":"../motif/src"}"#,
    );
    assert!(
        result.contains("agent.rs"),
        "Should find agent.rs: {}",
        result
    );
    assert!(
        result.contains("provider.rs"),
        "Should find provider.rs: {}",
        result
    );
}
...
fn test_search_content_mode() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"StopCondition","mode":"content","path":"../motif/src","glob":"*.rs"}"#,
    );
    assert!(
        result.contains("StopCondition"),
        "Should find StopCondition: {}",
        result
    );
}
...
fn test_search_files_with_matches() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"pub struct Agent","mode":"files_with_matches","path":"../motif/src","glob":"*.rs"}"#,
    );
    assert!(
        result.contains("agent.rs"),
        "Should list agent.rs: {}",
        result
    );
}
...
fn test_search_count() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"fn","mode":"count","path":"../motif/src","glob":"*.rs"}"#,
    );
...
fn test_search_pagination() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"use","mode":"filename","path":"../motif/src","head_limit":2,"offset":0}"#,
    );
    assert!(!result.is_empty());
    let all = common::call_tool(
        &tool,
        r#"{"query":"use","mode":"filename","path":"../motif/src","head_limit":0}"#,
    );
    assert!(!all.is_empty());
}
...
fn test_search_case_insensitive() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"stopcondition","mode":"content","path":"../motif/src","glob":"*.rs","ignore_case":true}"#,
    );
    assert!(
        result.contains("StopCondition") || result.to_lowercase().contains("stopcondition"),
...
fn test_search_nonexistent_path() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"x","path":"/nonexistent/path","mode":"filename"}"#,
    );
...
fn test_search_excludes_build_dirs() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"hello build","mode":"content","path":"_test_build"}"#,
    );
    assert!(
        result.contains("visible.txt"),
        "Should find visible.txt: {}",
        result
    );
    assert!(
        !result.contains("test.pyc"),
...
fn test_search_multiline_regex() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"struct \\w+ \\{[^}]*\\}","mode":"content","path":"_t_ml.txt","multiline":true}"#,
    );
...
fn test_search_globstar_pattern() {
    ...
    let result = common::call_tool(
        &tool,
        r#"{"query":"*.rs","mode":"filename","path":"../motif/src","glob":"*.rs"}"#,
    );
