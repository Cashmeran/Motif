...
impl RetryServer {
    ...
    async fn new(responses: Vec<(u16, &'static str)>) -> Self {
        ...
        tokio::spawn(async move {
            loop {
                let (mut socket, _) = match listener.accept().await { Ok(s) => s, Err(_) => break };
                let idx = c.fetch_add(1, Ordering::SeqCst);
                let (status, body) = if idx < resp.len() { resp[idx] } else { resp.last().copied().unwrap_or((200, "{}")) };
                let body = if body == "{}" {
                    format!(r#"{{"choices":[{{"message":{{"content":"retry_ok_{}"}},"finish_reason":"stop"}}]}}"#, idx)
                } else { body.to_string() };
                let response = format!(
        ...
        Self { addr, request_count: counter }
    }
}
...
async fn test_provider_retry_on_429() {
    ...
    assert!(result.message.content.contains("retry_ok"), "Should succeed after retry: {:?}", result.message.content);
    assert!(server.request_count.load(Ordering::SeqCst) >= 2, "Should have made multiple requests");
}
...
async fn test_provider_no_retry_on_400() {
    ...
    assert_eq!(server.request_count.load(Ordering::SeqCst), 1, "Only 1 request for 4xx");
}
